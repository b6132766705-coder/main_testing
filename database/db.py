    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute('''CREATE TABLE IF NOT EXISTS clans
                           (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, 
                            owner_id INTEGER, balance INTEGER DEFAULT 0, 
                            multiplier REAL DEFAULT 1.0, level INTEGER DEFAULT 1)''')
        
        await db.execute('''CREATE TABLE IF NOT EXISTS clan_requests 
                           (user_id INTEGER, clan_id INTEGER, PRIMARY KEY (user_id, clan_id))''')
